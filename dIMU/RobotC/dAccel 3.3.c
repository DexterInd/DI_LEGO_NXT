...
long accl_axis_reading_10(ubyte reg){
  ...
  writeDebugStream("LSB: ");
  writeDebugStreamLine("%x", LSB);
  writeDebugStream("MSB: ");
  writeDebugStreamLine("%x", MSB);
  ...
  writeDebugStream("Read: ");
  writeDebugStreamLine("%ld", axis_reading);
...
task main(){
  ...
  while (true){
      ...
      //long x_acc = accl_axis_reading_10(0x00);         // Get x axis data.
      //long y_acc = accl_axis_reading_10(0x02);         // Get y axis data.
      long z_acc = accl_axis_reading_10(0x04);         // Get z axis data.

      eraseDisplay();

      //float i_x_acc = ((float)(x_acc))/div;
      //float i_y_acc = ((float)(y_acc))/div;
      float i_z_acc = ((float)(z_acc))/div;
