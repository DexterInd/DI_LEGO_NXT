...
void calibrate_accel(){
  ...
  // This number changes depending on the g-force.
  z_drift = (z_drift-0x3F);                      // Placed right-side-up and perpendicular to the ground, axis should read 0x3F
  //z_drift = (z_drift-0x1F);                      // Placed right-side-up and perpendicular to the ground, axis should read 0x3F

  // Does the multiplier here change between g readings?
  // All of this works really well for 2g, but 4g doesn't seem to work very well.

  byte x_returned = accl_axis_write(0x10, (-x_drift));  // Write all the drifts to register.
  byte y_returned = accl_axis_write(0x12, (-y_drift));
  byte z_returned = accl_axis_write(0x14, (-z_drift));

  wait10Msec(100);
}

void calibrate_accel3(){
  byte x_off, y_off, z_off;
  ...
  x_off = accl_axis_reading_8(0x06);     // Read the x-axis reading.
  y_off = accl_axis_reading_8(0x07);     // Read the y-axis reading.
  z_off = accl_axis_reading_8(0x08);     // Read the z-axis reading.
