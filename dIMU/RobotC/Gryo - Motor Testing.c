...
int divisor = 128;      // This will be the divisor we divide the raw value of the gryscope by
                        // to get a scaled value on output.  Default will be for 250 dps, but we'll define it
                        // again in start_gyro.
...
task main(){
  ...
  while (true){
        ...
        //y_val = gyro_axis_reading(0x01);    // Get y-axis in dps.
        //z_val = gyro_axis_reading(0x02);    // Get z-axis in dps.
