...
void calibrate_accel(){
  ...
  byte x_returned = accl_axis_write(0x10, (-2*x_drift));  // Write all the drifts to register.
  byte y_returned = accl_axis_write(0x12, (-2*y_drift));
  byte z_returned = accl_axis_write(0x14, (-2*z_drift));

  wait10Msec(100);
}

task main(){
  g_mode = 0x01;                // Here we define the G-Mode we're going to use.
                                // 0x00 = +/- 2g
                                // 0x01 = +/- 4g
                                // 0x02 = +/- 8g
